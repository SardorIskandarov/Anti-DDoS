...
static void test_end_to_end_success(void) {
    ...
    rc = service_stats_wire_all(&g_active_arr);
    CHECK(rc == 0, "active stats_wire_all OK");
    ...
    /* Active arr should still have 44 active slots wired. */
    CHECK(g_active_arr.n_active == 44,
          "active_arr.n_active = %zu (expect 44)",
          g_active_arr.n_active);

    /* Spot-check that wiring transferred — first active slot should
     * have non-NULL detection_state + temporal_state. */
    int wired_count = 0;
    for (size_t i = 0; i < g_active_arr.capacity; i++) {
        if (g_active_arr.slots[i].active &&
            g_active_arr.slots[i].detection_state != NULL &&
            g_active_arr.slots[i].temporal_state  != NULL) {
            wired_count++;
        }
    }
    CHECK(wired_count == 44,
          "44/%d active slots have both det+tmp state wired",
          wired_count);
}
...
int main(void) {
    ...
    service_stats_unwire_all(&g_active_arr);
