...
/* Second accumulation bank for the hot path's double buffer. RX writes the
 * currently-active bank; the 1 Hz tick flips between the two so the snapshot
 * reads a frozen window with no concurrent writers. Both banks carry the
 * identical slot layout. */
static struct service_stats_array g_active_stats_array_b;
					...
					if (lcore_id == rte_get_main_lcore()) {
                        ...
                        if (service_reload_pending()) {
                            int rrc = service_reload_perform(
                                ...
                                &g_active_stats_array_b);
                            if (rrc == SERVICE_RELOAD_OK) {
                                /* Both banks were reinitialised from the new
                                 * registry; resume accumulation on bank0. */
                                service_hotpath_reset_active_bank();
                            } else if (rrc != SERVICE_RELOAD_ERR_NO_FLAG) {
                                fprintf(stderr,
...
main(int argc, char **argv)
{
	 ...
	 * load is intentionally observe-only at P3. */
	{
		...
		if (sr_rc == SERVICE_REGISTRY_OK) {
			...
			/* Allocate BOTH runtime stats banks (raw counters + sketches)
			 * for the hot path's double buffer. Detection state lives in the
			 * Python brain post-cutover, so there is no per-slot
			 * detection/temporal state to wire here. */
			int rc = service_stats_array_init(&g_active_stats_array);
			if (rc == 0)
				rc = service_stats_array_init(&g_active_stats_array_b);
			if (rc != 0) {
			...
			rc = service_stats_init_from_registry(&g_active_stats_array,
			                                       &g_registry_storage);
			if (rc == 0)
				rc = service_stats_init_from_registry(&g_active_stats_array_b,
				                                       &g_registry_storage);
			if (rc != 0) {
				rte_exit(EXIT_FAILURE,
			...
			printf("[main] service_stats double-buffer allocated for %zu slots\n",
			 ...
			 * flip + publish + reset via service_hotpath_tick() at 1Hz. */
			if (service_hotpath_init(&g_registry_storage,
			                          &g_active_stats_array,
			                          &g_active_stats_array_b) != 0) {
				rte_exit(EXIT_FAILURE,
	...
	/* Tear down both stats banks. Must happen BEFORE the registry detach so
	 * no stray hot-path reader could observe a registry pointing at freed
	 * slots. */
	service_stats_array_destroy(&g_active_stats_array);
	service_stats_array_destroy(&g_active_stats_array_b);
