...
/**
 * One-time hot-path init. Caches the registry and the TWO double-buffered
 * accumulation banks. RX lcores write into the currently-active bank; the 1 Hz
 * tick flips the active bank, lets in-flight writers drain, then reads + resets
 * the now-frozen bank — so the snapshot sees a window with no concurrent
 * writers. Both banks MUST already be initialised (array_init +
 * init_from_registry) and have identical slot layout.
 *
 * Must be called AFTER service_registry_set_global() and AFTER both stats
 * arrays are initialised, but BEFORE the packet loop starts.
 *
...
/**
 * Reset the active bank to bank0. Call on the main lcore after a SIGHUP reload
 * has reinitialised both banks from the new registry, so accumulation resumes
 * on a known-clean bank with the new slot layout.
 */
void service_hotpath_reset_active_bank(void);

/**
 * Tear down the hot path. Unmaps the snapshot shm and clears cached pointers.
 * Idempotent.
 */
...
/**
 * Per-1Hz tick. Called from the main-lcore timer block in l2fwd_main_loop().
 *
 *   1. Flip the active accumulation bank (RX lcores switch to the other bank).
 *   2. Settle briefly so in-flight RX writers drain off the now-frozen bank.
 *   3. Publish the frozen window's RAW telemetry to the snapshot shm for the
 *      Python detection brain, then reset its per-window counters.
 */
