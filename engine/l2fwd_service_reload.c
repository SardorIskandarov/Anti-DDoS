 ...
 *   5. (Post-cutover: no per-slot detection/temporal state to wire — the
 *      staging array holds raw telemetry only.)
 *   6. Publish staging_reg via service_registry_set_global() (release).
 *   7. Tear down old active_arr (destroy).
 *   8. Copy *staging_reg into *active_reg and *staging_arr into *active_arr.
 *      The heap allocations (slots) transfer by pointer-copy — the staging
 *      structs are zeroed afterwards so destroy
...
int service_reload_perform(
    ...
    struct service_stats_array   *active_arr_b)
{
    ...
    if (!staging_reg || !staging_arr || !active_reg || !active_arr ||
        ...
        record_failure("NULL argument to service_reload_perform "
                       "(staging_reg=%p staging_arr=%p active_reg=%p "
                       "active_arr=%p active_arr_b=%p)",
    ...
    /* (Post-cutover: no per-slot detection/temporal state to wire — the
     * Python brain owns detection; the staging array holds raw telemetry only.) */
     ...
     * Shallow copy is correct: the heap pointer inside service_stats_array
     * (slots[]) transfers ownership by pointer-copy. We then zero the staging
    ...
    *active_arr = *staging_arr;
    ...
    /* --- Step 8b: reinitialise the SECOND double-buffer bank from the new
     * registry so both banks carry the new slot layout. Its old accumulation
     * is discarded (a config change invalidates per-slot indices anyway). The
     * caller flips the hot path back to bank0 via reset_active_bank(). */
    service_stats_array_destroy(active_arr_b);
    rc = service_stats_array_init(active_arr_b);
    if (rc == 0)
        rc = service_stats_init_from_registry(active_arr_b, active_reg);
    if (rc != 0) {
        record_failure("second-bank reinit failed: rc=%d", rc);
        return SERVICE_RELOAD_ERR_ALLOC;
    }
