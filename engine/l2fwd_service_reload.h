 ...
 * @param active_arr    active stats bank 0 (BSS-stable address)
 * @param active_arr_b  active stats bank 1 — the hot path's second
 *                      double-buffer bank; reinitialised from the new registry
 *                      so both banks share the new slot layout. The caller must
 *                      reset the hot path's active bank afterwards
 *                      (service_hotpath_reset_active_bank).
 * @return SERVICE_RELOAD_OK on success, negative error code otherwise.
 *
 * NOTE: this is NOT yet coordinated with hot-path readers dropping their
 * references to old slots (no RCU grace period) — a known limitation carried
 * over from P6; reloads are rare and operator-driven.
 */
